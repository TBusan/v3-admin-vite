  ...
  <div>
    <div class="event-info">
      <div class="event-info-row">
        ...
        <label class="event-back-text">单击一次时</label>
      </div>
        ...
        <el-collapse-item name="1">
          <template #title>
            <el-icon class="custom-icon-add" @click="addEvent('move')" v-show="roleMoveControlList.length == 0"
              ><Plus
            /></el-icon>
            <el-icon class="custom-icon-subtract" @click="subtractEvent('move')" v-show="roleMoveControlList.length > 0"
              ><Minus
            /></el-icon>
            控制角色移动
          </template>
          ...
          <div class="custom-select">
            <div v-for="item in modelControlList" :key="item.id">
              <div class="custom-select-lable">
                ...
                <el-icon class="custom-select-lable-close" @click="closeTheItem(item, 'model')"><CloseBold /></el-icon>
              </div>
        ...
        <el-collapse-item name="4">
          ...
          <div class="custom-select">
            <div v-for="item in mediaControlList" :key="item.id">
              <div class="custom-select-lable">
                <label> {{ item.name }}</label>
                <el-icon class="custom-select-lable-close" @click="closeTheItem(item, 'media')"><CloseBold /></el-icon>
              </div>
...
let temp_animationPathListOptions: any = null //动画路径
let temp_pathListOptions: any = null //路径包括了动画路径
...
let activeTypeValue = "singleClick"
...
const addEvent = (params: string) => {
  if (params === "move") {
    ...
    //路径控制
    if (!temp_animationPathListOptions) {
      temp_animationPathListOptions = getAllPathAnimation()
    }
    const item = {
      id: 1,
      name: "路径控制",
      pathValue: "",
      pathListOptions: temp_animationPathListOptions || [],
      pathActionValue: "",
...
const selectSceneChange = (item: any) => {
  ...
  let viewPointListOptions: any[] = [] //视点与路径
  let pathListOptions: any[] = [] //路径
  debugger
  if (pageId == pageInfoStore.getSelectPageInfo.id) {
    //当前页面
    viewPointListOptions = getAllViewPoint()
    pathListOptions = getAllPath()
    viewPointListOptions = temp_viewPointListOptions.concat(pathListOptions)
  } else {
    //其它页面的视点
    // viewPointListOptions = getAllViewPointFromOtherPage(pageId)
    // //其它页面的路径
    // pathListOptions = getAllPathFromOtherPage(pageId)
    viewPointListOptions = getAllViewAndPathFromOtherPage(pageId)
  }
...
//获取其它页面的视点与路径
const getAllViewAndPathFromOtherPage = (pageId: string) => {
  let tempList: any[] = []
    ...
    if (findPageItem) {
      ...
      if (allFeatureList && allFeatureList.length > 0) {
        allFeatureList.map((item: any) => {
          if (
            item?.vault?.type.toLowerCase() == "camera" ||
            item?.vault?.type.toLowerCase() == "mheight" ||
            item?.geometry?.type.toLowerCase() == "linestring" ||
            item?.geometry?.type.toLowerCase() == "polygon"
          ) {
            tempList.push({
              value: item?.id,
              label: item?.name
...
//从其它页面获取路径
const getAllPathFromOtherPage = (pageId: string) => {
  console.log("getAllPathFromOtherPage")
  let tempList: any[] = []
  return tempList
}
            ...
            for (let k = 0; k < findItemEvenInfo.actionList.length; k++) {
              if (findItemEvenInfo.actionList[k].type == "roleMove") {
                ...
                if (!temp_pathListOptions) {
                  temp_pathListOptions = getAllPath()
                }
                let tmpList = temp_viewPointListOptions.concat(temp_pathListOptions)
                const item = {
                  id: k + 1,
                  name: "移动位置",
