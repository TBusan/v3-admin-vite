<template>
  <div class="card-container-div card-container-div-top-two" id="layerListCardContainerId">
    <el-card v-if="!isCollapse" shadow="never" class="card-container-layer">
      ...
      <el-input v-model="filterText" @change="filterTheLayers" placeholder="图层筛选的关键字" @keydown.stop />
        ...
        >
          <template #default="{ node, data }">
            <div class="custom-tree-node" @dblclick="handleDblclick(data)">
              <div class="icon-span">
                <svg-icon
                  v-if="data.data.itemtypeStr.toLowerCase() == 'media'"
                  class="udstream-tool-svg"
                  name="layer-photo"
                />
                <svg-icon
                  v-else-if="data.data.itemtypeStr.toLowerCase() == 'camera'"
                  class="udstream-tool-svg"
                  name="layer-camera"
                />
                <svg-icon
                  v-else-if="data.data.itemtypeStr.toLowerCase() == 'folder'"
                  class="udstream-tool-svg"
                  name="layer-folder"
                />
                <svg-icon
                  v-else-if="data.data.itemtypeStr.toLowerCase() == 'maptile'"
                  class="udstream-tool-svg"
                  name="layer-map"
                />
                <svg-icon
                  v-else-if="data.data.itemtypeStr.toLowerCase() == 'poi' && data?.data?.projectNode.geomtype == 1"
                  class="udstream-tool-svg"
                  name="layer-location-item"
                />
                <svg-icon
                  v-else-if="data.data.itemtypeStr.toLowerCase() == 'poi' && data?.data?.projectNode.geomtype == 3"
                  class="udstream-tool-svg"
                  name="layer-measure-line"
                />
                <svg-icon
                  v-else-if="data.data.itemtypeStr.toLowerCase() == 'poi' && data?.data?.projectNode.geomtype == 5"
                  class="udstream-tool-svg"
                  name="layer-measure-area"
                />
                <svg-icon
                  v-else-if="data.data.itemtypeStr.toLowerCase() == 'mheight'"
                  class="udstream-tool-svg"
                  name="layer-measure-vertical"
                />
                <svg-icon
                  v-else-if="data.data.itemtypeStr.toLowerCase() == 'places'"
                  class="udstream-tool-svg"
                  name="VectorCollect"
                />
                <svg-icon v-else-if="data.data.itemtype == 10" class="udstream-tool-svg" name="layer-model-item" />
                <svg-icon
                  v-else-if="data.data.itemtypeStr.toLowerCase() == 'uds'"
                  class="udstream-tool-svg"
                  name="layer-uds"
                />
                <svg-icon
                  v-else-if="data.data.itemtypeStr.toLowerCase() == 'qfilter'"
                  class="udstream-tool-svg"
                  name="layer-filter"
                />
              </div>
...
const listLoading = ref(true) //列表界面的加载

const cameraFly2PathShow = ref(false) //相机绕线段飞行的显示

const filterText = ref("") //树节点的过滤
...
const handleDragEnd = (draggingNode: Node, dropNode: Node, dropType: NodeDropType, ev: DragEvents) => {
  ...
  let parentPtr = 0
  let insertBeforePtr = 0
  let nodePtrArr = nodes.map((node) => node.data?.projectNodePtr ?? 0)
  udStream.moveSceneNodes(nodePtrArr, parentPtr, insertBeforePtr)
}
...
//查询对应的图层数据
const filterTheLayers = (params: string) => {
  console.log(params)
  if (params) {
    //执行查询操作
    let search = params.trim().toLowerCase()
    if (threeLayerTreeData.value) {
      threeLayerTreeData.value = threeLayerTreeData.value.filter((node) => {
        let isFilteredSearch =
          node.title.toLowerCase().includes(search) || node.data?.itemtypeStr.toLowerCase().includes(search)
        return isFilteredSearch
      })
    }
  } else {
    threeLayerTreeData.value = allTheTreeNodes
  }
}

//找到转场的数据进行Pinia的存储
let cameraChildrenList: ISlTreeNodeModel<NodeData>[] = []
const findCameraData2Pinia = (nodes: ISlTreeNodeModel<NodeData>[] | undefined) => {
  if (nodes && nodes.length > 0) {
    for (let k = 0; k < nodes.length; k++) {
      if (nodes[k]?.data?.itemtypeStr.toLowerCase() == "camera") {
        cameraChildrenList.push(nodes[k]) //相机
      }
      if (nodes[k].children && nodes[k]?.children.length > 0) {
        findCameraData2Pinia(nodes[k].children)
      }
    }
  }
}
...
//有被点中模型
let findUdstreamNode = null
//找到场景中对应的uudd
const findClickNodeUUID = (treeNodes: ISlTreeNodeModel<NodeData>[] | undefined, uuid: string) => {
...
onBeforeMount(() => {
  ...
  udStream.registerGetClickedItemCallback((uuid, isMultiSelect) => {
    // 照理要对目录树进行反选
    // console.log("udstream根节点信息发生改变......registerGetClickedItemCallback----3", uuid, isMultiSelect)
    if (uuid) {
      findClickNodeUUID(allTheTreeNodes, uuid)
      if (findUdstreamNode) {
...
watch(
  ...
  (nv, ol) => {
    console.log("udstreamStore.getUdstreamRootNode.UUID--1")
    if (setTimeoutIndex) {
    ...
    setTimeoutIndex = setTimeout(() => {
      if (nv && nv.name) {
        // console.log("udstreamStore.getUdstreamRootNode.UUID--2")
        allTheTreeNodes = assembleTheTreeNode(nv)
        cameraChildrenList = []
        if (allTheTreeNodes) {
          threeLayerTreeData.value = allTheTreeNodes
          findCameraData2Pinia(allTheTreeNodes)
          udStreamCameraStore.setItem(UdstreamCameraInfoStoreEnums.UDSTREAM_CAMERA_INFO, cameraChildrenList)
        } else {
          //恢复图层的节点信息
          threeLayerTreeData.value = []
          //恢复相机的节点信息
          udStreamCameraStore.setItem(UdstreamCameraInfoStoreEnums.UDSTREAM_CAMERA_INFO, [])
        }
      }
